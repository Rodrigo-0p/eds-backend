...
exports.main = async (req, res, next)  => {
  ...
  const {valor,cod_marca} = req.body;
  try {
      var sql =   `select *
                     from (
                   select p.rowid id, p.cod_linea, p.descripcion desc_linea
                    ...
                    where p.cod_empresa = :cod_empresa
                      ...
                      and (p.cod_linea like '%' || :valor || '%' or upper(p.descripcion) like '%' || upper(:valor) || '%' or :valor = 'null')
                      and nvl(p.estado, 'I') = 'A'
                    order by lpad(p.cod_linea, 10, '0') asc )
                    where rownum <= 100
                  `;
    var data = {cod_empresa,cod_marca,valor};
